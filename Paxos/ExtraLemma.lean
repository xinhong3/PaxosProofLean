...
/-- The immediate consequence of `QuorumAssumption`. It is used to pick a witness from the quorum intersection. -/
@[simp]
...
/-- If max_prop is not empty (`some v`), then the acceptor must has voted for that value `v`.-/
@[simp]
...
This is used in later proof when we need to do pattern matching on both ballot and value for `Message.twob`, and since they are both `Option`, we need to cover the following cases:
...
-- Effort: 20 min
/-- If the message from `max_prop` has empty ballot and value, then the acceptor has not voted in any previous ballots. Used in proving the case when both `rbal` and `rval` are empty in the proof of `Phase1b`. -/
@[simp]
...
/-- This is the immediate consequence of the definition of `Send`. We use this lemma to show that `Next` grows the history variable `sent` monotonically. -/
@[simp]
...
/-- Corresponds to the monotonicity of existantial quantifier describe in the paper. We combine this lemma with previous monotonic results to show some property `P` of the history variable holds inductively. -/

...
-- Helper definitions for checking message types.
...
/-- `none + 1 ≤ some b` for any ballot `b`. Used in showing inductiveness for `Phase2a`. -/
@[simp]
lemma ballot_none_plus_one_leq_ballot {b : Ballot} :
    (none : Option Ballot) + (1 : Nat) ≤ some b := by
  dsimp [HAdd.hAdd]
  exact Nat.zero_le b
...
/-- Transitive property of `Option Ballot`. Used in showing inductiveness for `Phase2a`. -/
@[simp]
...
@[simp]
lemma phase1a_imp_mono_sent {b : Ballot} {sent sent' : Set Message}
    (hPhase1a : Phase1a sent sent' b) : sent ⊆ sent' := by
  exact send_monotonic hPhase1a

/-- `Phase1b` grows `sent` monotonically. -/
@[simp]
lemma phase1b_imp_mono_sent {a : Acceptor} {sent sent' : Set Message}
...
@[simp]
lemma phase2a_imp_mono_sent {b : Ballot} {sent sent' : Set Message} {Quorums : Set (Set Acceptor)}
...
@[simp]
lemma phase2b_imp_mono_sent {a : Acceptor} {sent sent' : Set Message}
    (hPhase2b : Phase2b sent sent' a) : sent ⊆ sent' := by
  rcases hPhase2b with ⟨m2b, hm2b, hmatch⟩
  cases m2b <;> simp at hmatch; simp [hmatch]

-- Lemmas about monotonicity of `VotedForIn` and `WontVoteIn`. We use word "stable" to mean
-- implication, and "same" to mean if and only if.
...
lemma oneb_inv_2_stable_if_no_twob_added {maxVBal : Option Ballot} {b : Ballot} {a : Acceptor}
  ...
  exact h_no_vote_in_sent_b' v
    ((votedForIn_same_if_no_2b_added sent sent' h_mono h_no_2b_added).mpr h_voted_in_sent')

...
/-- Message invariants for 1b, 2a, and 2b messages. We added an extra invariant for 1b
because `maxVBal` and `maxVal` are `Option` types. We need to say if they are not both
`some ... `, then they must both be `none`. -/
def MsgInv : Prop :=
  ∀ m ∈ sent,
    match m with

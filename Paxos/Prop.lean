...
def MsgInv : Prop :=
  ∀ m ∈ sent, match m with
    | Message.oneb b maxVBal maxVal a => (
      match maxVBal, maxVal with                             -- ! in the TLA proof this is matched with ballot number (-1) and otherwise. We match it with both, and added another invariant that is, when one of them is none, the other is also none.
        | some rbal, some rval => VotedForIn sent a rval rbal
        | option_rbal, option_rval => option_rbal = none ∧ option_rval = none)          -- ADDED in the Lean proof
      ∧
      (∀ (b' : Ballot), ((b' ≥ maxVBal + (1: Nat)) ∧ (b' < b)) → ¬ (∃ v : Value, VotedForIn sent a v b')) -- This works even if `maxVBal` is none, because we defined `LE` for `Ballot` and `Option Ballot`, see Spec.lean
